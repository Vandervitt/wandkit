...
上一轮我猜是 `position: sticky`。实测否定了这个判断——真正的机制是
**Element UI 开启固定列时会把整个表格 DOM 复制一份**：

```
详情按钮总数: 40   （主表格 20 + .el-table__fixed-right 副本 20）
```

主表格那 20 个位于 `x=1378`，而固定列容器覆盖 `1204~1444`，因此**被完全盖住**；
`elementFromPoint` 在该点命中的是副本层的「更多」按钮。

更棘手的是副本自身也不可用：它在 `overflow: hidden` 容器里布局，
`getBoundingClientRect` 返回 `x=808`（错位），三点采样全部落在别处。

于是**两份都被排除**，整列操作按钮对 Agent 消失。

### 解法：命中栈而非栈顶

`elementsFromPoint` 返回该点上的完整命中栈。实测数据给出了决定性区分：

| | `elementFromPoint` 命中自己 | 在 `elementsFromPoint` 栈中 |
|---|---|---|
| 主表格按钮 | ❌（命中「更多」） | ✅（栈深 23） |
| 弹窗遮罩下的元素 | ❌ | ❌ |

「元素是否出现在栈中」既保住了本职——弹窗遮罩下的元素根本不在栈里——又不再误伤
**同位置重叠**（固定列副本、粘性表头）。两者的区别正是：
...
无条件调用 `elementsFromPoint` 会把整体耗时从 21ms 拉到 **167ms**（2985 节点页面）。
改为先试单点、未命中再取栈——绝大多数元素本来就在栈顶，这条几乎总是命中。
...
### 顺带修掉 scrollable 的第二处丢弃

`scrollable` 此前只豁免了「无名纯容器」规则，还有一条「无名且无可辨识属性的非语义
元素」规则同样会丢弃它——滚动容器天然无名无属性。补上豁免后，
`.el-table__body-wrapper` 正确报为「可滚动区域（横向）」。

### 排查方法论

这一轮再次印证：**猜测机制远不如实测**。我最初认定是 sticky，实际是 DOM 复制；
若照猜测去写「识别 sticky 定位」的修复，会完全无效。
