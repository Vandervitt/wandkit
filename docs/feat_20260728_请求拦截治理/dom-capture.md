...
| 5 | 扩充 cursor 集合 + 事件监听器兜底 + 可滚动容器 | 拖拽、纯 JS 绑定元素可被发现 | ✅ 完成 |
...
前面几轮只验证了「静态页面的快照质量」。这一轮触发真实交互，验证两个此前从未被触发
过的能力。
...
```
调价 / 登录其他账号 / 导出话机密码 / 同步 / 操作记录 / 停用
```

6 个新选项全部带 `*` 标记。模型据此能直接判断「刚才那次点击展开了这些」，无需自己
比对两次快照。
...
底层 62 个元素全部正确排除，快照只剩弹窗内容。这直接印证了 §2.1 的判断——**确认
卡片本身就是遮罩层**，闸门弹出期间模型不该看到底层元素。

### 由此发现的三个缺陷

对比截图与快照逐项核对时发现弹窗少了控件，顺藤摸瓜挖出三个独立缺陷：

**① `role="spinbutton"` 不在白名单**（根因，最严重）

Element UI 的数字输入框渲染成 `<input type="text" role="spinbutton">`，而
`spinbutton` 是 W3C 标准 role。因不在白名单，4 个单价框**全部对 Agent 隐形**，模型
根本无法填写调价表单。

显式 role 优先于标签推断，所以「它本来就是 `<input>`」兜不住这个洞。已按 WAI-ARIA
补齐全部 widget role（新增 `spinbutton` / `menuitemcheckbox` / `menuitemradio` /
`treeitem` / `gridcell`）。

> **白名单漏一个 role，等于整类控件对 Agent 隐形。** 这类缺陷不会报错、不会崩溃，
> 只会让 Agent 安静地做不到某件事——是最难从现象反推的一种。

**② `tabindex` 被当成可点击**

Element UI 的 tooltip 图标是 `<span class="el-tooltip" tabindex="0">`，仅凭
`tabindex` 被判成 button；它又嵌在标签文字里，于是整条表单项被当成按钮，把真正该
操作的输入框吞掉。

修复：`tabindex` 只说明「可聚焦」，还需渲染成可点（`cursor: pointer`）才算按钮。

**③ `<label>` 被当成可点目标**

排查①时顺带修掉：`<label>` 永远只是说明文字，不该成为可点目标。它作为名字来源的
职责不受影响。

### 修复后的弹窗快照

```
*[2] textbox    请输入客户模式 = "后付" (readonly)
*[3] textbox    请选择计费方式 = "单独计费" (readonly)
*[4] spinbutton 请输入AI呼出单价 = "0.01"
*[5] spinbutton 请输入AI呼入单价 = "0.01"
*[6] spinbutton 请输入短信单价 = "0"
*[7] spinbutton 请输入坐席费
*[8] textbox    请输入调价备注
*[9] button     确 定
*[10] button    取 消
...
语义标签 + ARIA role + `cursor: pointer` 覆盖了绝大多数情况，仍有三类漏网。

### 扩充 cursor 集合

新增 `move` / `grab` / `grabbing`——拖动排序把手、可拖拽面板同样是渲染出来的交互
承诺，用户看到这个光标就知道能动。

**刻意不含 `text`**（page-agent 收了它）：普通段落大量使用 `cursor: text`，收进来会
让整页文字涌入快照。可编辑区域另有 `contenteditable` 与 `<textarea>` 兜住。

### 事件监听器兜底

老后台大量存在 `<div onclick="doSomething()">` 且不给指针样式的写法，不认它就整块
功能对 Agent 隐形。查 `onclick` / `onmousedown` / `onkeydown` 等内联属性。

要求祖先没有同类属性——事件委托极常见，全收会让一次点击产出一堆候选。

**已知缺口**：`addEventListener` 绑定的监听器在标准 DOM API 下不可枚举
（`getEventListeners` 仅存在于 DevTools 控制台），因此这条兜底只覆盖属性写法。

### 可滚动容器

新增自造 role `scrollable`。页面级 `scroll` 到不了内部滚动区，不识别的话它下半截的
内容对 Agent 永远不存在。

判据是「溢出取值允许滚动 **且** 实际有可滚动距离（>8px）」——只看 `overflow` 会把
满页面内容未超出的容器都报成可滚动；阈值是为了避开亚像素与边框圆整。

两处配套修改：

- **豁免「无名纯容器」规则**。滚动区的价值恰恰在于它是个容器，模型要的是「这里还能
  往哪边翻」。真实表格实测：横向可滚 570px 的 `.el-table__body-wrapper` 因无自身
  文本被该规则整个丢弃。
- **`scroll` 原语补上 `index` 参数**，并首次暴露为工具。此前 `PageController.scroll`
  存在但从未注册成工具，模型根本调不到——识别出滚动区却无法操作等于没做。

### 遗留：sticky 固定列的遮挡误判

真实列表页实测发现一个**既有问题**（非本轮引入）：表格操作列是 `position: sticky`
固定列，`elementFromPoint` 在该位置命中的是另一个重叠的按钮，导致行内操作按钮被
遮挡判定排除。

| 配置 | 元素数 | 「详情」按钮 |
|---|---|---|
| 默认（开遮挡） | 68 | ❌ 被排除 |
| `detectOcclusion: false` | 95 | ✅ |

当前 68 个元素仍可用（表单、分页、部分行内按钮在列），但固定列的操作按钮缺失。
需要在遮挡判定中识别 sticky/fixed 定位的重叠情形，留作后续修复。
