...
| 5 | 自动化发现探针（见下） | 零声明拿到能力目录 | 待办 |
| 6 | ExecutorPort + page-agent 适配器 | 端到端：自由操作被闸门拦下 | 待办 |
| 7 | trace 接入 + README 定位改写 | 审计闭环，文档与实现一致 | 待办 |
...
| 探针 | 实现要点 | 依赖 |
|---|---|---|
| **请求观测** | 拦截器已看到每个请求，累积 `method + 归一化 path` 成 API 目录 | 阶段 2 完成即可，**近乎免费** |
| **路由表** | 从宿主 `RouterPort` 取全部路由；需扩展该端口（现仅有 `getCurrentRouteName` / `push`） | 端口小改 |
| **可访问性树扫描** | 扫当前页可交互元素，取 `role` + 可访问名；**禁止**用 class / 组件名（见 design §4.4） | 无 |

path 归一化是这里的核心难点：`/api/users/u_1` 与 `/api/users/u_2` 必须收敛成 `/api/users/:id`，否则目录会随用量无限膨胀。可复用 `compileUrlPattern` 的反向逻辑（阶段 1 已实现 glob→正则，此处需要 实例→模式）。

冷启动优先接 OpenAPI（若宿主有），可直接生成名单基线，是零声明成本拿到完整写入面的最优路径。
