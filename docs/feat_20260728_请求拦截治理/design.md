...
**不再要求接入方声明能力清单。** 能力由运行时探针自动发现。

#### 三个探针

| 探针 | 回答什么 | 覆盖范围 | 生产构建后可靠性 |
|---|---|---|---|
| **可访问性树扫描** | 当前页**能点什么** | 仅当前页 | ✅ 见 §4.4 |
| **路由表** | 系统**有哪些页面** | 全局 | ✅ 路由名是配置字符串 |
| **请求观测（拦截器）** | 实际**发生过什么写** | 全局，累积式 | ✅ method + path 不被 mangle |

第三个探针是**免费的**：拦截器为了治理本就要看每个请求，顺手把 `method + 归一化 path` 累积成 API 目录即可。治理与发现复用同一装置，不必另造探测机制。

#### 关键限制：发现能力 ≠ 发现后果

自动发现能告诉你「这里有个按钮叫**删除用户**」，但告诉不了你：删的是哪个、影响多少条、可不可逆。**后果在请求里，不在按钮上。**
...
这也再次印证了闸门必须落在请求层：DOM 侧永远无法可靠判定危险程度。

#### 冷启动

请求观测是累积式的，首次运行为空。三种补充手段，按可得性优先：

1. **OpenAPI / Swagger 文档**（若有）——机器可读的完整写入面，可直接生成名单基线：所有非安全方法进「需确认」，人工只需标注例外。这是**零声明成本**拿到完整目录的最优路径。
2. **路由表 + 首次页面扫描**——覆盖 UI 可达的能力。
3. **纯冷启动**——全部走默认拒绝。安全但初期确认较频繁，随观测积累收敛。
